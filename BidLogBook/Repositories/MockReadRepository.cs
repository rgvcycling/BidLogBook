...
namespace BidLogBook.Repositories
{
    public class MockReadRepository : IReadRepository
    {
        ...
        public IEnumerable<LogBook> GetLogsList()
        {
            var logBook = _bidLogBooks;

            return logBook;
        }
        public IEnumerable<LogBook> GetLog(int logId)
        {
            var logBook = _bidLogBooks.Where(uc => uc.LogId.Equals(logId));
            return logBook;
        }
    }
}
